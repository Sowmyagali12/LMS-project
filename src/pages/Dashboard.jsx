...
import { FaBook, FaFileAlt, FaLaptopCode, FaBullhorn, FaClock } from "react-icons/fa";
...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-100 text-gray-800">
      ...
      {/* Header Section */}
      <div
        className="relative overflow-hidden rounded-3xl mx-auto mt-8 mb-12 p-6 text-white shadow-lg w-[95%] max-w-7xl flex flex-col md:flex-row items-center gap-6"
        style={{
      ...
      >
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-bold mb-1 tracking-tight">
            Welcome, {student?.name || "gurram"}!
          </h1>
          <p className="text-blue-100 text-base">
            Good day! Here’s a quick overview of your program and recent progress.
          </p>
        </div>
      </div>

      {/* Dashboard Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-[95%] max-w-7xl mx-auto mb-12">
        {dashboardCards.map((card, idx) => (
          <Link key={idx} to={card.link}>
            <div className="group relative bg-white/90 backdrop-blur-md border border-blue-100 p-5 rounded-2xl shadow hover:shadow-lg transition-all duration-300 hover:-translate-y-2">
              <div className="flex justify-between items-center mb-3">
                <div className="text-lg text-black transition-transform duration-300 group-hover:scale-110">
                  {card.icon}
                </div>
                {card.trend && <span className="text-sm text-gray-500">{card.trend}</span>}
              </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-blue-100 p-8 w-[95%] max-w-7xl mx-auto mb-12">
        ...
        <div className="space-y-4">
          {recentActivity.map((activity, i) => (
            <div key={i} className="flex items-start gap-4 bg-blue-50/50 p-4 rounded-xl shadow-sm hover:shadow-md transition-all duration-300">
